...
function Color({id, title, color, rating, onRemove = f => f}) {
    return (
        <section>
            ...
            <StartRatingComponent selectedStars={rating}/>
        </section>
    );
}

export default function ColorListComponent({colors_list = [], onRemoveColor = f => f}) {
    if (!colors_list.length) return <div>No Colors Listed. (Add a Color)</div>;
    return (
        <div>
            <h2>Lista de colores</h2>
            {
                colors_list.map(color => <Color key={color.id} {...color} onRemove={onRemoveColor}/>)
            }
        </div>
    );
}